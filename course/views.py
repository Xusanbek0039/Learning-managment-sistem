...
def program_view(request):
    ...
    return render(request, 'course/program_list.html', {
        'title': "Yo'nalish | IT Creative LMS",
        'programs': programs,
    })
...
def program_add(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, request.POST.get('title') + ' muvafaqiyatli qo\'shildi,')
    ...
    return render(request, 'course/program_add.html', {
        'title': "Yo'nalish qo'shish | IT Creative LMS",
        'form': form,
    })
...
def program_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, str(request.POST.get('title')) + ' yo`nalish yangilandi.')
            return redirect('programs')
    else:
        form = ProgramForm(instance=program)

    return render(request, 'course/program_add.html', {
        'title': "Yo'nalish taxirlash | IT Creative LMS",
        'form': form
    })
...
def program_delete(request, pk):
    ...
    title = program.title
    program.delete()
    messages.success(request, 'Yo\'nalish nomi ' + title + ' muvafaqiyatli o\'chirildi')
...
def course_add(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, (course_name + '(' + course_code + ')' + ' muvafaqiyatli yaratilindi.'))
    ...
    return render(request, 'course/course_add.html', {
        'title': "Kurs qo'shish | IT Creative LMS",
...
def course_edit(request, slug):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, (course_name + '(' + course_code + ')' + ' muvafaqaiyatli qo\'shildi.'))
            return redirect('program_detail', pk=request.POST.get('program'))
        else:
            messages.error(request, 'Quyidagi xato(lar)ni tuzating.')
    else:
    ...
    return render(request, 'course/course_add.html', {
        'title': "Kurs taxrirlash | IT Creative LMS",
...
def course_delete(request, slug):
    ...
    course.delete()
    messages.success(request, 'Kurs nomi: ' + course.title + ' muvafaqiyatli o\'chirildi.')
...
class CourseAllocationFormView(CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['title'] = "Kursga saylash | IT Creative LMS"
        return context
...
def course_allocation_view(request):
    ...
    return render(request, 'course/course_allocation_view.html', {
        'title': "Kurs taqsimlash | IT Creative LMS",
...
def edit_allocated_course(request, pk):
    ...
    return render(request, 'course/course_allocation_form.html', {
        'title': "Ajratilgan kursni tahrirlash | IT Creative LMS",
...
def handle_file_upload(request, slug):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, (request.POST.get('title') + ' muvafaqiyatli yangilandi.'))
            return redirect('course_detail', slug=slug)
    else:
        form = UploadFormFile()
    return render(request, 'upload/upload_file_form.html', {
        'title': "Fayl yuklash | IT Creative LMS",
...
def handle_file_edit(request, slug, file_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, (request.POST.get('title') + ' muvafaqiyatli o\'chirildi.'))
            return redirect('course_detail', slug=slug)
    else:
...
def handle_file_delete(request, slug, file_id):
    ...
    messages.success(request, (file.title + ' muvafaqiyatli o\'chirildi.'))
    return redirect('course_detail', slug=slug)
...
def handle_video_upload(request, slug):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, (request.POST.get('title') + ' muvafaqiyatli yuklandi..'))
            return redirect('course_detail', slug=slug)
    else:
        form = UploadFormVideo()
    return render(request, 'upload/upload_video_form.html', {
        'title': "Video yuklash | IT Creative LMS",
        'form': form, 'course': course
    })
...
def handle_video_edit(request, slug, video_slug):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, (request.POST.get('title') + ' muvafaqiyatli taxrirlandi.'))
            return redirect('course_detail', slug=slug)
    else:
...
def handle_video_delete(request, slug, video_slug):
    ...
    messages.success(request, (video.title + ' muvafaqiyatli o\'chirildi.'))
    return redirect('course_detail', slug=slug)
# ########################################################
...
def course_registration(request):
    if request.method == 'POST':
        ...
        for s in range(0, len(ids)):
            ...
            obj.save()
            messages.success(request, 'Kurslar muvaffaqiyatli ro\'yxatdan o\'tgan!')
